...
#GBM
fit<- gbm(Result~.,data=Maths,distribution = "multinomial",n.trees = 500,shrinkage = 0.05,interaction.depth = 3,bag.fraction = 0.5,keep.data = FALSE,cv.folds = 5)
nTrees <- gbm.perf(fit)
print(nTrees)
print(summary(fit))
...
test_answer$Answer<-gsub(".62","",test_answer$Answer)
